"""CLI entry point for AI-SystemAssist."""

import argparse
import os
import sys

from ai_systemassist.server import run_server, DEFAULT_HOST, DEFAULT_PORT


def main():
    parser = argparse.ArgumentParser(
        prog="ai-systemassist",
        description="Start the AI-SystemAssist command execution server. "
    ...
    parser.add_argument(
        "--host",
        default=os.environ.get("AI_SYSTEMASSIST_HOST", DEFAULT_HOST),
    ...
    parser.add_argument(
        "--port", "-p",
        type=int,
        default=int(os.environ.get("AI_SYSTEMASSIST_PORT", DEFAULT_PORT)),
        help=f"Port to listen on (default: {DEFAULT_PORT})",
    )
    parser.add_argument(
        "--token",
        default=os.environ.get("AI_SYSTEMASSIST_TOKEN"),
    ...
    if args.version:
        from ai_systemassist import __version__
        print(f"ai-systemassist {__version__}")
        sys.exit(0)
